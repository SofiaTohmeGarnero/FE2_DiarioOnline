window.onload = () => {
    cargarNoticiasEnWeb(noticias);
    noticiasIntenacionales(noticias);
...
const noticias = [
    ...
    function renderizarNoticia(noticia){
        ...
        `
        carrete.innerHTML += template;
    }

    function cargarNoticiasEnWeb(noticias){
        noticias.forEach(n => {
            renderizarNoticia(n);
        });
    }

    function noticiasIntenacionales(noticias){
        const contenedoresNoticia = document.querySelectorAll('.contenedorNoticia');
        noticias.forEach((n,i) =>{
            if(n.tipoNacional==false){
                contenedoresNoticia[i].classList.add('internacional')
            } 
        })
    }
